...
> 字体规则：**Karla 单字体系统；Gotu 仅用于页面一级标题**（如 Account Profile）；弹窗标题使用 Karla SemiBold 20px / weight 600
> 设计基调：**功能优先 · 信息密度高 · 极简数据驱动 · 与官网同源但更克制**
> 版本：**v5.0** · 2026-06-15 · Engage 模块四级 Tab（Dashboard / Signal Feed / Keywords & Accounts / Replies）落地版
...
弹窗标题        → Karla Bold 600  （"Create Post" / "Edit Time Slots" / "Add Channel"）
...
> **标题字体发生变化**：figma 中各弹窗标题统一 Karla SemiBold 20px / weight 600（不是 Gotu、也不是 Bold 700）。Gotu 仅出现在**页面顶部一级标题**中（参见 6.15）。
...
| icon 容器 | 46×46，白底，圆角 10，`shadow: 0 1px 2px rgba(17,17,17,0.06)` | 内置**拟物 mascot 插画**（紫色立体 chat-bubble 角色 + 点状眼），资源 `engage-aisee-v2/engage-banner-icon.svg`（= `uploads/engage.svg`）。**⚠️ 不要用扁平描边 chat-bubble outline** |
...
- **7 个紧凑 pill**（v5.1：从 3 张大卡改为紧凑 pill，省约一半高度），每个 = 前导 icon(14×14) + 标题，**无副标**：
  - `Expert answer`（list-checks）· `Data-backed`（bar-chart）· `Empathetic`（heart）· `Contrarian`（corner-up-left）· `Ask a question`（help-circle）· `Quick take`（zap）· `Amplify`（volume-2）
- 布局：`flex flex-wrap gap-6`，pill 高 32、padding `0 10px`、圆角 8、Karla 12.5/500，自动换行（400px 面板约每行 3 个）
  - 未选：白底 + `rgba(17,17,17,0.05)` 描边，icon `rgba(17,17,17,0.55)`
  - **选中**：`#FFE253` 黄底 + 黑色 1px 描边，icon `#111`
- **副标不丢**：pill 组下方一行 hint（ⓘ + `{选中项标题}` — `{副标}`），始终显示当前选中策略的完整说明（如 "Expert answer — Structured steps"）；hover 任意 pill 另有原生 tooltip
- 宽 widget（≥700px standalone）语境可改回带副标的 `repeat(auto-fit, minmax(150px,1fr))` 卡片网格；窄面板统一用紧凑 pill
...
- 子 tab 下方的 **工具行随子 tab 变化**：
  - **Sent / All**：一组平台筛选 pill `X 3` / `Reddit 2`（22 高 chip + 黑底数字徽章，多选）
  - **Awaiting review（v5.2 新）**：一整行 `AwaitingToolbar`——左侧平台**切换按钮**、右侧状态**分段筛选器**，见 §13.7.5
...
#### 13.7.5 Awaiting review 子 tab（v5.2 重建 · Review Queue）

> 实现来源：figma "Engage- Sent- draft" + "Engage- Sent-Awaiting link" frame（33:29747 / 33:29911）。实现：`engage-aisee-v2/replies.jsx`。
> **v5.1 曾把 Awaiting review 定义为极简单行 ReviewRow，已废弃**。v5.2 起它是一个完整的「待办回复队列」——AI 起草但尚未发布 / 已发布但缺链接 / 已过期的回复都集中在这里处理，带内部状态筛选和到期预警。

**结构（自上而下）**：`AwaitingToolbar` → `ExpiryBanner` → 分区标题（Drafts / Awaiting link / Expired）→ 卡片列表（`DraftCard` / `LinkNeededCard`）。列表 gap 10。

##### AwaitingToolbar（平台切换 + 状态筛选，一行）
- 一行 flex，`padding 8px 32px`，底部 `1px solid rgba(17,17,17,0.05)`；左侧平台按钮 `flex-grow`，右侧状态筛选器 `shrink-0`
- **平台切换按钮**（与 Sent 的 pill 不同，是带 count 徽章的按钮，单选）：
  - 高 28，圆角 8，padding `0 12`，gap 6
  - **active**：黑底 `#111` + 白字 + 白 glyph；count 徽章底 `rgba(255,255,255,0.12)` / 白字
  - **inactive**：白底 + 描边 `inset 0 0 0 1px rgba(17,17,17,0.05)` + `#111` 字；count 徽章底 `rgba(17,17,17,0.04)` / `#111`
  - count 徽章：圆角 999，min-w 16，padding `0 5`，Karla 10–12 / opacity 0.8 / tabular-nums
  - 例：`X 10`（active）/ `Reddit 2`
- **状态分段筛选器**（segmented control）：
  - 容器高 30，圆角 8，底 `rgba(17,17,17,0.03)`，padding 3，gap 2
  - 每项：高 24，圆角 6，padding `0 8`，gap 6 = 文字（Karla 12 / 500）+ count Overlay pill（圆角 999，min-w 16，padding `0 5`，Karla 12）
  - **active 项**：白底 `#FFFFFF` + `box-shadow: 0 1px 2px rgba(0,0,0,0.08)`，文字 `#111`；count 底 `rgba(17,17,17,0.06)`
  - **inactive 项**：透明底，文字 `rgba(17,17,17,0.6)`；count 底 `rgba(17,17,17,0.04)` / `rgba(17,17,17,0.7)`
  - 四项固定：`All 10` / `Drafts 6` / `Awaiting link 3` / `Expired 1`（`Drafts`=尚未发布的草稿，`Awaiting link`=已发布待回填链接，`Expired`=超 7 天）

##### ExpiryBanner（到期预警条 · 仅 Drafts / All 且有临期草稿时出现）
- 圆角 16，`padding 15px 12px`，gap 8，items-center
- 背景 **暖金渐变**：`linear-gradient(94.245deg, rgba(255,166,41,0.14) -2.29%, rgba(237,187,37,0.14) 99.67%)`
- 左：32×32 白底圆角 6 容器 + bell icon 18（`#EC5212`）
- 文案两行：`{N} drafts expire within 24h`（Karla 600 12 / lh 18 / `#111`）+ `post them before the 7-day reply window closes.`（Karla 400 12 / lh 18 / `#111`）

##### 分区标题行（Drafts）
- 左：`DRAFTS`（Karla 11 / 600 / uppercase / letter-spacing 0.06em / `rgba(17,17,17,0.55)`）
- 右：`Sorted by time left — the most urgent drafts are first.`（Karla 11 / muted）；`Awaiting link` / `Expired` 分区右侧改为 `{N} replies` 计数

##### DraftCard（AI 已起草、尚未发布）
> 单卡两层：**外壳白底**（圆角 12，padding 4，`box-shadow: inset 0 0 0 1px rgba(17,17,17,0.05)`）包一个 **content 区**（圆角 `8px 8px 0 0`，底 `rgba(17,17,17,0.03)`，**dashed 描边 `1px dashed rgba(17,17,17,0.05)`**，padding 8，gap 8）+ 底部 action 行。

- **头部一行**（h24）：
  - 左组：sender avatar 24（圆，`inset 1px rgba(17,17,17,0.05)`）+ sender handle（Karla 12 / 500 / `#111`）+ `↩` 回复箭头 16（灰、水平翻转）+ recipient avatar 24 + `@handle`（Karla 12 / 500 / **`#8C7400`**）+ 平台 glyph 14（X `#111` / Reddit `#EC5212`）
  - 右组（gap 8）：**到期徽章** + **Draft 徽章** + **`Drafted 6d ago`**（Karla 12 / 500 / `#5C6D7A`）
    - **到期徽章**（h20，圆角 8，padding `1px 8px`，gap 4）：
      - 临期（≤24h）→ 底 `rgb(252,231,231)` 浅红 + clock icon + 文字，全 `#EC5212`，例 `Expires today` / `Expires tomorrow`
      - 普通（>24h）→ 底 `rgb(247,239,227)` cream + clock icon + 文字，全 `#3D3D3A`，例 `Expires in 3 days`
      - 已过期 → 底 `#FFFADD` + `#111` ✕ 圆环 icon + `Expired`（`#111`）
    - **Draft 徽章**（h20，圆角 999，padding `0 8`，gap 4）：底 `#E5E6EC` + textbox/pencil icon 12 + `Draft`，文字 `#5C6D7A` / 500 / 12
- **From 引用块**：左 2px 竖分隔条 `rgba(17,17,17,0.05)` + 右列：
  - 行 1：`From`（Karla 13 / 600 / `#111`）+ 原帖 snippet（Karla 12 / 400 / `rgba(17,17,17,0.5)`，单行 ellipsis）
  - 行 2：`Posted Jun 4, 2026 · 10:30 AM`（Karla 12 / 400 / `rgba(17,17,17,0.6)`）
- **草稿回复框**（区别于 Sent，专属**奶黄底 + 黄描边**）：
  - 圆角 10，底 **`#FFFEF5`**，`box-shadow: inset 0 0 0 1px rgba(255,226,83,0.6)`，padding `8px 12px`，gap 4
  - 头行：`Your reply`（Karla 10 / 600 / `#5C6D7A` / opacity .8）+（多版本时）**版本翻页 pill**：底 `rgba(17,17,17,0.03)`，圆角 999，h22，`◄ 1 / 2 ►`（chevron 12 + `n / total` Karla 11 / 500 / tabular-nums；到边界 chevron opacity 0.32）
  - 正文：Karla 12 / 500 / lh 16 / `#111`，`-webkit-line-clamp: 3`
  - `See more ▾`（Karla 12 / 400 / underline / `rgba(17,17,17,0.5)`）
- **action 行**（`padding 4px 8px 8px`，gap 8，items-center）：
  - `Expert answer` 策略 tag（圆角 32，底 `rgba(17,17,17,0.03)`，padding `6px 8px`，Karla 12 / 500 / `#3D3D3A`）；多版本时显示当前版本的 angle
  - `flex-1` spacer
  - `Open in Signal Feed`（secondary：白底 + `inset 1px rgba(17,17,17,0.05)`，外链 icon + 文字，h28 / 圆角 8）
  - `Copy & Open X`（**黑底 `#111` + 白字**，X glyph 白，h28 / 圆角 8）
  - `Post via Aisee Extension`（**黄底 `#FFE253` + 黑字**，X glyph 黑，h28 / 圆角 8）
  - **Expired 卡**：`Copy & Open X` / `Post via Aisee Extension` 两个主 CTA `disabled`（opacity 0.5），整卡 opacity 0.92

##### LinkNeededCard（已发布、等待回填链接）
> 外壳同 DraftCard，但 content 区用**实线描边**（`1px solid rgba(17,17,17,0.05)`）区别于 Draft 的 dashed。

- **头部一行**：avatars + handles（同上）+ **`Link needed` 徽章**（h20，圆角 8，padding `1px 8px`，底 `rgba(144,120,0,0.1)` + link/attachment icon 12 + 文字，全 `#8C7400`）+ `Drafted 16h ago`（`#5C6D7A`）
- **From 引用块**：时间前缀改为 `At`（`At Jun 4, 2026 · 10:30 AM`）——因为回复已发出
- **已发布回复**（inline，无奶黄框、无翻页）：`Your reply` label（Karla 10 / 600 / `#5C6D7A`）+ 正文 Karla 12 / 500 / clamp 3 + `See more ▾`
- **action 行**：
  - **状态 pill** `Link to reply not submitted · Data pending update`：底 `rgba(144,120,0,0.1)`，ⓘ icon + 文字，全 `#8C7400`（Karla 12 / 500，圆角 8，h26）
  - `Expert answer` 策略 tag（同上）
  - `flex-1` spacer
  - `Open Thread`（secondary，外链 icon）
  - `Submit Reply URL`（**黄底 `#FFE253` + 黑字**，link icon 黑，h28 / 圆角 8）——回填链接主 CTA
- 不显示 stats grid（回复数据尚未回填）

#### 13.7.6 Reply Tracking（追踪功能 · v5.1 新增）

> 实现：`engage-tracking.jsx`（组件）+ `engage-replies-tracking.jsx`（页面集成）。给已发回复增加「持续追踪互动数据」能力，分**免费插件式**与**付费托管式**两档。仅出现在 **已提交链接的 Sent 回复**上。

**两种追踪模式**：
| 模式 | 触发 | 计费 | 频率 | 状态色 |
|---|---|---|---|---|
| **Plugin tracking**（插件式） | 浏览器扩展打开页面时刷新 | **免费** | 被动（仅扩展访问时） | 灰 `rgba(17,17,17,0.05)` chip |
| **Hosted tracking**（托管式 · 推荐） | aisee 云端 24/7 监控 | **5 credits / reply / day**（按日递减，见费率表） | 智能自动调整 | 紫 `--purple-soft` 底 / `#4327a3` 字 + 跳动 live dot |

**托管监控费率表**（成本随贴龄递减，写进 overview 侧栏）：0–24h 每 1h（5cr/天）→ 24–48h 每 12h（3cr）→ 48–72h 每 24h（2cr）→ Day4–7 每 24h（1cr）→ Day8+ 停止。首周预估 ≈ 35 credits。

**三个使用场景**：
1. **主动追踪入口**：未追踪的 Sent 卡片在 `rep-foot` 最右侧显示 `◉ Track` 按钮（追踪眼 icon）→ 打开**居中弹窗**「Track this reply」，含两档选择 + 7 天预估 + Cancel / 黑色主按钮。
2. **发布后追问**：提交链接成功后弹出庆祝 modal「Reply published!」，顺势询问是否追踪（同两档选择）。
3. **状态与管理**：已追踪卡片显示**状态 chip**（Hosted = 紫底跳动点 + `5 cr/day` / Plugin = 灰底 Free）+ `Manage ▾` → 打开「Tracking details」弹窗（method / started / frequency / spent today / total + 操作按钮）。

**Tracking overview 侧栏**（账户级）：仅在 **All / Sent** 子 tab **常驻显示**（不需按钮触发），宽 312。含 hosted 计数、预估 credits/天、今日消耗、剩余 credits、Top up 按钮、监控费率表。切到 Awaiting / Dismissed / Failed 时自动隐藏、列表恢复全宽。

**弹窗按钮铁律（v5.1 强调）**：所有弹窗内按钮**一律纯填充、无描边**——
- 主按钮：黑底 `#111` / 白字
- 次按钮（Cancel / Use free / Switch to plugin）：浅灰填充 `rgba(17,17,17,0.06)` / `#111` 字
- **危险按钮（Stop hosted tracking）：填充 `#EC5212` / 白字 `#ffffff`**
- 文字过长时按钮可上下排版（stacked，整行宽），但样式不变

**追踪眼图标**：紫色拟物 chat-bubble 角色 + 黄色下眼睑 + 黑色瞳孔（pupil 左右巡视 + 周期眨眼的纯 CSS 动画）。资源同 Engage Banner mascot。
...
| **（v5.2）Draft 徽章底** | **`#E5E6EC`** + `#5C6D7A` 字 | — |
| **（v5.2）Link needed 徽章 / 状态 pill** | 底 **`rgba(144,120,0,0.1)`** + `#8C7400` 字 / icon | — |
| **（v5.2）临期到期徽章（≤24h）** | 底 **`rgb(252,231,231)`** + `#EC5212` clock/文字 | — |
| **（v5.2）普通到期徽章（>24h）** | 底 **`rgb(247,239,227)`** cream + `#3D3D3A` | — |
| **（v5.2）草稿回复框** | 底 **`#FFFEF5`** + `inset 0 0 0 1px rgba(255,226,83,0.6)` 黄描边 | — |
| **（v5.2）到期预警条渐变** | **`linear-gradient(94.245deg, rgba(255,166,41,0.14) -2.29%, rgba(237,187,37,0.14) 99.67%)`** | — |
| **（v5.2）Expert answer 策略 tag** | 底 `rgba(17,17,17,0.03)` + `#3D3D3A` 字 | — |
...
> 文档版本：**v5.0** · 2026-06-15 · § » v5.1 · figma 反向校对 · sidebar height / banner 色 / area chart / donut 修正
>
...
> - **【v5.1】Replies Tracking（§13.7.6）**：新增 Plugin（免费）/ Hosted（5cr/reply/day，费率递减）双档追踪；三场景（Track 弹窗 / 发布后追问 modal / 状态 chip + Ma
> - **【v5.2】Replies · Awaiting review 重建（§13.7.5）**：从 v5.1 的单行 ReviewRow 升级为**待办回复队列**——`AwaitingToolbar`（平台切换按钮 + `All / Drafts / Awaiting link / Expired` 分段筛选器）+ `ExpiryBanner` 暖金渐变到期预警条 + `DraftCard`（奶黄草稿框 + 版本翻页 + 三个发布 CTA：Open in Signal Feed / Copy & Open X / Post via Aisee Extension）+ `LinkNeededCard`（Link needed 徽章 + 状态 pill + Submit Reply URL）。新 token：Draft 徽章 `#E5E6EC`、Link needed `rgba(144,120,0,0.1)`/`#8C7400`、临期徽章 `rgb(252,231,231)`、草稿框 `#FFFEF5`+黄描边。实现：`engage-aisee-v2/replies.jsx`。nage 弹窗）；All & Sent 常驻 Tracking overview 侧栏。**弹窗按钮一律纯填充无描边**——主黑 / 次浅灰 / 危险 `#EC5212` 白字。Engage Banner icon 更正为**插画 mascot**（`engage-aisee-v2/engage-banner-icon.svg`），不再用扁平 outline。
