...
- 入口是否全部注册、参考文件是否存在。
- 使用门户真实提示词模板与配置读取器，检查所有入口两模式只在 Goal 不同、默认无参数、显式变体白名单、页面/hook/API/配置保护及范围外先确认。
...
至少对每个带变量的组件执行：默认无变体复制 → 切换仅动效模式 → 改一个预览变量（仍无目标）→ 菜单明确选择目标变体后复制 → 关闭相关开关 / 换组合（旧选择清除）→ 重新选择后复制。把实际复制文本与可见控件状态逐项对照。检查同页独立示例、隐藏控制器、切页加载时复制，以及刷新后的默认值。
