...
现行交互（2026-09-24）：Copy for AI 默认 Apply AISEE design，另可选 Add motion only；两模式共享组件文件与业务保护。默认不携带任何预览配置，只有用户在复制菜单的 Target variant 区明确选中的一个变体才附加。切换预览后清除旧变体，切换组件后恢复默认模式与无变体。预览快照仍按下文读取，但不等于用户接入意图。


所有带变量的组件：按需调整预览，在复制菜单的 Target variant 区中明确选择一项后，再点 **Copy for AI** 才附加该配置；不选择则仅复制组件目标与范围。此规则不只适用于 Sidebar。更改选择后需要重新复制；复制结果不会自动同步到其他 AI，也不会自动改变产品默认布局。
...
- 同页多个示例是独立配置，不能合并成单个组件；门户只附加用户明确选中的一项，未选时不附加。
...
React 示例从实际状态生成 `data-aisee-config` JSON；静态示例在 `assets/component-config.js` 注册显式读取函数。门户在预览加载/交互后读取配置以展示可选变体，并在复制时重新读取校验，防止陈旧选择。不得用通用遍历所有 input 的方式代替配置声明。
