...
- 每次收到用户请求后，在回答或开始执行任务前，必须先评估完成当前请求所需的大致处理时间，并将预计耗时放在该次首次对用户可见回复的最开头。
- 统一使用简短格式：
  - `预计耗时：不到 1 分钟`
  - `预计耗时：约 1–3 分钟`
  - `预计耗时：约 5–10 分钟`
- 预计耗时应综合考虑任务复杂度、执行步骤、工具调用、代码修改、文件处理等当前请求内的工作量，不要求精确，只需给出合理区间。
- 告知预计耗时后直接继续执行任务，不需要等待用户确认，也不要为了估算时间额外向用户提问。
- 执行过程中，仅当任务范围、阻塞情况或剩余工作量发生明显变化，导致原预计耗时明显不再准确时，才更新预计耗时。
- 对于需要连续执行多个步骤的任务，预计耗时应覆盖整个当前任务，而不是只估算下一步操作。
