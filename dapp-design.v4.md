...
| **Post Agent** | yellow | `#FFE253` | Calendar / Channels / Models / Media / **Engage**（新模块） |

> **Engage 位置说明**（v4 新增）：Engage 是 Post Agent zone 下的一个二级功能模块（Sidebar v5.0.2 `Property1Postagent2`），**主色沿用 yellow `#FFE253`**（所有 step badge 、active tab、subreddit input 均用 yellow）；但默认 Banner 背景用独有的 lavender-pink `#F3E7F4` 作为模块身份色。
...
> 文档版本：v4.0 · 2026-05-22
> 关键更新（v4 · Engage 模块扩充）：
> - 新增 **Engage 模块**（归在 Post Agent zone 下，主色沿用 yellow `#FFE253`）
> - 新增 Engage 专属身份色 lavender-pink `#F3E7F4`（仅 Banner）
> - 新增调色 token：`#E6F0CD/#597300` keyword pill、`#FFFADD` subreddit 背景、`#FF4500` Reddit 品牌色、`#A5D500` sync dot 光环
> - 新增 11 个 Engage 专用组件：Banner / PrimaryTab / SecondaryTab / Numbered Step Badge / Keyword Pill / Subreddit Card / Generate Reply Drawer / Mark Replied Panel / Copied Toast / Success Toast / Engage 页面模板
>
> 旧版本变更（v3.1 · Comparison Modal 实战修订）：
