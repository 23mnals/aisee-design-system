...
Notification 已提供短版源码交付试点：选好变量后复制并粘贴给可联网、可编辑项目和运行 Node 的编码 AI，它会自动获取完整组件。目标项目需支持 React 18+、TSX 与 CSS/SVG 资源；无需先装 AISEE 整库。详见 [Notification 单组件接入](docs/NOTIFICATION_COPY_AI.md)。其他组件暂沿用现有方式。
