...
import { EmptyStateIllustration, getEmptyStateActionTone, type EmptyStateActionTone, type EmptyStateIllustrationName } from './EmptyStateIllustration';
...
export interface EmptyStateProps extends Omit<HTMLAttributes<HTMLDivElement>, 'title' | 'children'> {
  ...
  variant?: 'plain' | 'inset';
  titleAs?: 'h2' | 'h3' | 'h4' | 'p';
  actionTone?: EmptyStateActionTone | 'auto';
}
...
export function EmptyState({
  ...
  size = 'default', variant = 'plain', titleAs: Title = 'h2', actionTone = 'auto', className = '', ...props
}: EmptyStateProps) {
  const id = useId();
  ...
  const usesBuiltInIllustration = illustration === undefined && illustrationName !== undefined;
  const resolvedActionTone = actionTone === 'auto'
    ? usesBuiltInIllustration ? getEmptyStateActionTone(illustrationName) : 'neutral'
    : actionTone;
  ...
  return <div
    ...
    data-aisee-action-tone={resolvedActionTone}
    {...props}
