...
- 精简底部位置选择文案，说明路由切换示例用途，并替换门户顶部不协调的分享图标。
...
- Overview、Schedule、Replies 上方增加用途说明，明确它们只模拟页面切换并验证 Runner 持续显示。
- Share 改为链环图标，继续执行复制当前页面链接。
...
- 组件 Demo 构建与生产交付生成通过。
- 浏览器实测标题、NEW、Show runner、Default、Right 均保持单行；Demo 页面切换用途可见，Share 链环图标正常显示。
