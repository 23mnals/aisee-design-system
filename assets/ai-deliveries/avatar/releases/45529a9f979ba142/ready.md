# Integrate AISEE Avatar

Production delivery version: 45529a9f979ba142. Installer SHA-256: ea600a112807edc4fcea5869a553d45e018c62e462a940ec19b8c92371bbb749
...
- assets/avatar/platform-x-mark.svg
- assets/avatar/social-avatar-set.svg
- index.ts
- styles.css.d.ts
- assets/avatar/dapp-avatar-set.svg.d.ts
- assets/avatar/platform-x-mark.svg.d.ts
- assets/avatar/social-avatar-set.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/avatar/releases/45529a9f979ba142/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 ea600a112807edc4fcea5869a553d45e018c62e462a940ec19b8c92371bbb749 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/avatar` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/avatar` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
