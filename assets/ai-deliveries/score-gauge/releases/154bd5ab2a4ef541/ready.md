# Integrate AISEE ScoreGauge

Production delivery version: 154bd5ab2a4ef541. Installer SHA-256: 25c5e9bcf53714c73945631756602db31ea67faa9e765b88725c8610f3450228
...
- ScoreGauge.tsx
- styles.css
- assets/score-gauge/score-gauge-texture.png
- index.ts
- styles.css.d.ts
- assets/score-gauge/score-gauge-texture.png.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/score-gauge/releases/154bd5ab2a4ef541/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 25c5e9bcf53714c73945631756602db31ea67faa9e765b88725c8610f3450228 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/score-gauge` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/score-gauge` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
