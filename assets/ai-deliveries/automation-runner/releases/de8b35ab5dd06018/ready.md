...
Production delivery version: de8b35ab5dd06018. Installer SHA-256: e76a6ae77d1136d636e826af302c9dcb41cbc2c38d44ca0c8b66e7ebb703ec06
...
- button: AutomationRunner.tsx. Required: native button semantics, refs, accessible labels and keyboard activation without host default visuals; AISEE controlled visibility/view state, drag handle pointer capture, focus rings and playful motion. Fallback: standalone.
...
- Preserve default, expanded and minimized views, controlled open/view callbacks, real detail rows and the Back to Automation host callback. The green eye restores a minimized runner; the header expands details; the line control minimizes it.
- Before integration, inspect the host for an existing Automation Runner or equivalent persistent status card. Update a compatible existing implementation in place instead of mounting a second runner. Retain an existing blink and pointer-following eye only when they already behave correctly; do not duplicate their DOM, state, keyframes or window pointermove listener.
- Except for a compatible existing blink and pointer-following eye, this delivery is authoritative for all component behavior: bottom entrance and exit bounce, default/expanded/minimized views, pointer-directed card lean, one jelly rebound only when entering the green mascot, whole-card pointer dragging, keyboard drag-handle movement, viewport clamping and complete prefers-reduced-motion fallback. A drag must not activate expand, minimize, close or action controls.
...
1. Complete Host Project Compatibility inspection above, then download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/automation-runner/releases/de8b35ab5dd06018/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 e76a6ae77d1136d636e826af302c9dcb41cbc2c38d44ca0c8b66e7ebb703ec06 before execution. Do not print archive data into chat.
2. Run `node .aisee-install.cjs src/components/aisee/automation-runner` (adapt the path). It unpacks exactly the production list; it installs no npm packages and refuses different existing files. Inspect conflicts or choose a new directory; never erase previous integrations automatically.
