# Integrate AISEE NotificationBell

Production delivery version: b9054a32dfa37f1d. Installer SHA-256: 06f048932240442be782fab5f12f4cbac3b35a9ae30294da89131137a3d4ff93
...
- button: NotificationBell.tsx. Reference capabilities (only the requested missing subset applies to an existing target): native button props, accessible label, pointer/focus and click callbacks; AISEE inner glyph motion and badge state driven by real count; outer button stays fixed. Fallback for an absent target only: standalone.
...
- Keep the inline bell icon, hover/focus, numeric badge and optional unread dot.
- Real count increases ring the white bell and animate the badge number. Rerenders, unchanged/decreased/cleared counts must not replay incoming animation.
- The outer button and black icon circle remain fixed; respect prefers-reduced-motion. Connect count and onClick to existing product data and navigation.
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/notification-bell/releases/b9054a32dfa37f1d/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 06f048932240442be782fab5f12f4cbac3b35a9ae30294da89131137a3d4ff93 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/notification-bell` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/notification-bell` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
