# Integrate AISEE TooltipToast

Production delivery version: 0f8b6436f11929ea. Installer SHA-256: e625b15ba75c8824d7fab94f039f1f69c31f21c2eae2a04a19dee02669c1eba8
...
- tooltip: Tooltip.tsx. Reference capabilities (only the requested missing subset applies to an existing target): hover/focus open, Escape dismiss and trigger ref; portal and collision-aware automatic placement with scroll/resize tracking; AISEE spring pointer tracking, directional entrance and separate positioning/motion transforms. Fallback for an absent target only: standalone.
- live-region: Toast.tsx. Reference capabilities (only the requested missing subset applies to an existing target): status/alert semantics, accessible dismissal and lifecycle cleanup; AISEE timer/progress and entry/exit motion hooks. Fallback for an absent target only: standalone.
...
- index.ts
- styles.css.d.ts
- assets/stemui/action-check.svg.d.ts
- assets/stemui/action-close.svg.d.ts
...
- Preserve Tooltip auto placement, viewport collision handling, hover/focus/Escape, subtle/playful spring motion and reduced motion.
- Preserve Toast dismissal and timed progress behavior; do not create a toast demonstration page.
...
Use a compatible React/TypeScript project and a bundler supporting CSS and SVG/image asset imports. Component compatibility (from this manifest): react >=18, node >=18. Node applies to running the source installer, not the browser component. Copy for AI imposes no additional global version requirement. Runtime dependencies: react >=18 (reuse host), react-dom >=18 (reuse host). If the host is not React, explain the stack mismatch; do not convert the whole application. For SSR use a client boundary around interactive components.

1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/tooltip-toast/releases/0f8b6436f11929ea/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 e625b15ba75c8824d7fab94f039f1f69c31f21c2eae2a04a19dee02669c1eba8 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/tooltip-toast` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/tooltip-toast` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
