# Integrate AISEE ToggleSelectionGroup

Production delivery version: bf5f4acbb445a3a1. Installer SHA-256: 5aeb4ce8e4da9ba70b1211b3d1c28de924c030333c137c137a5c570ec4c16f22
...
- selection-group: ToggleSelectionGroup.tsx. Reference capabilities (only the requested missing subset applies to an existing target): independent group enablement and preserved item selections; accessible labels, disabled handling and keyboard activation. Fallback for an absent target only: standalone.
- checkbox: Checkbox.tsx. Reference capabilities (only the requested missing subset applies to an existing target): checked, indeterminate and disabled state, form semantics; keyboard activation, focus and AISEE celebration state hooks. Fallback for an absent target only: standalone.
...
- ToggleSelectionGroup.tsx
- Checkbox.tsx
- Toggle.tsx
- styles.css
- assets/toggle-selection-group/check.svg
- index.ts
- styles.css.d.ts
- assets/toggle-selection-group/check.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/toggle-selection-group/releases/bf5f4acbb445a3a1/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 5aeb4ce8e4da9ba70b1211b3d1c28de924c030333c137c137a5c570ec4c16f22 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/toggle-selection-group` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/toggle-selection-group` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
