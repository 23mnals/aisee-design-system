# Integrate AISEE ConfirmationDialog

Production delivery version: ba6520530f7761d6. Installer SHA-256: d84206e6ddd07e2fa6b55025ea85dfe04c46c92b7ff112030b1c4623eb2ab69e
...
- dialog: ConfirmationDialog.tsx. Reference capabilities (only the requested missing subset applies to an existing target): controlled open, cancel-first focus, focus return and modal isolation; Escape/cancel handling, accessible title/description and native dialog ref API; AISEE impact notices, confirm-disabled behavior and host-owned callbacks. Fallback for an absent target only: standalone.
- button: Button.tsx. Reference capabilities (only the requested missing subset applies to an existing target): native button semantics, disabled/type and keyboard activation; forwarded button ref and AISEE className, icon slots and hover/focus hooks. Fallback for an absent target only: standalone.
...
- Preserve the controlled native modal, safe-action initial focus, focus containment and return, Escape/close/cancel callbacks, responsive actions and scrollable long content.
- Preserve optional positive/warning impact notices and host icon/content slots. Labels, notices, counts and confirm callbacks come from the host; never install the automation/discard examples or their illustration assets.
- The host decides when confirmation succeeds and closes the dialog; do not automatically run a business operation or add demo Toast behavior.
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/confirmation-dialog/releases/ba6520530f7761d6/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 d84206e6ddd07e2fa6b55025ea85dfe04c46c92b7ff112030b1c4623eb2ab69e before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/confirmation-dialog` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/confirmation-dialog` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
