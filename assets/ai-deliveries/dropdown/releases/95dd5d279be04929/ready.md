# Integrate AISEE Dropdown

Production delivery version: 95dd5d279be04929. Installer SHA-256: 9167abf98a5627ecb022d7a19160b76b40354c52bb3d594b65b43531cf2abb0d
...
- popover: Dropdown.tsx. Reference capabilities (only the requested missing subset applies to an existing target): controlled visibility, outside dismissal and focus return; anchored collision-aware positioning, scroll/resize handling and portal className. Fallback for an absent target only: standalone.
- selection: Dropdown.tsx. Reference capabilities (only the requested missing subset applies to an existing target): single/multiple selection, editable/filterable input and keyboard navigation; disabled/grouped items and nested row actions without selection conflicts. Fallback for an absent target only: standalone.
...
- Dropdown.tsx
- styles.css
- assets/dropdown/search.svg
- assets/stemui/action-check.svg
- assets/stemui/line_chevron-up.svg
- index.ts
- styles.css.d.ts
- assets/dropdown/search.svg.d.ts
- assets/stemui/action-check.svg.d.ts
- assets/stemui/line_chevron-up.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/dropdown/releases/95dd5d279be04929/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 9167abf98a5627ecb022d7a19160b76b40354c52bb3d594b65b43531cf2abb0d before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/dropdown` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/dropdown` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
