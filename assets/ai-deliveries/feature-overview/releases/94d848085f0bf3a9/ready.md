# Integrate AISEE FeatureOverview

Production delivery version: 94d848085f0bf3a9. Installer SHA-256: d1ccf6c6d2c8dd5b64d9339e75c25624499833ce3ca43f0344e67a70349b9f3f
...
- FeatureList.tsx
- styles.css
- assets/feature-overview/check-analysis.svg
- assets/feature-overview/check-engage.svg
- assets/feature-overview/check-growth.svg
- assets/feature-overview/check-publish.svg
- assets/stemui/line_chevron-up.svg
- index.ts
- styles.css.d.ts
- assets/feature-overview/check-analysis.svg.d.ts
- assets/feature-overview/check-engage.svg.d.ts
- assets/feature-overview/check-growth.svg.d.ts
- assets/feature-overview/check-publish.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/feature-overview/releases/94d848085f0bf3a9/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 d1ccf6c6d2c8dd5b64d9339e75c25624499833ce3ca43f0344e67a70349b9f3f before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/feature-overview` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/feature-overview` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
