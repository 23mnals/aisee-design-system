# Integrate AISEE QuantityStepper

Production delivery version: 9f6a975fc8f97bd9. Installer SHA-256: 041d6c48d98229e7cb920f95acc323e29bef082d88edb54a70df2882a886e5b2
...
- number-input: QuantityStepper.tsx. Reference capabilities (only the requested missing subset applies to an existing target): editable and buttons-only modes, bounds and step; press-and-hold repeat and keyboard input without duplicate handlers. Fallback for an absent target only: standalone.
...
- QuantityStepper.tsx
- styles.css
- assets/stemui/action-minus.svg
- assets/stemui/action-plus.svg
- index.ts
- styles.css.d.ts
- assets/stemui/action-minus.svg.d.ts
- assets/stemui/action-plus.svg.d.ts
...
- Preserve keyboard editing, hold acceleration, min/max boundaries and numeric roll motion.
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/quantity-stepper/releases/9f6a975fc8f97bd9/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 041d6c48d98229e7cb920f95acc323e29bef082d88edb54a70df2882a886e5b2 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/quantity-stepper` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/quantity-stepper` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
