# Integrate AISEE SidebarNavigation

Production delivery version: dba6b45d4384dc73. Installer SHA-256: 8d302f2bee70397e8fdd1e666d338218a245b3a444f2c0bff6068c37d5dcc5e6
...
- portal: SidebarNavigation.tsx. Reference capabilities (only the requested missing subset applies to an existing target): explicit portal target and host stacking context; toggle ownership, focus, Escape and hover reveal coordination. Fallback for an absent target only: standalone.
- tree-navigation: TreeNav.tsx. Reference capabilities (only the requested missing subset applies to an existing target): controlled selection and expansion, disabled nodes; nested keyboard behavior and AISEE hierarchy/state hooks. Fallback for an absent target only: standalone.
...
- Preserve collapse/reveal, nested navigation, layout variants and keyboard behavior.
...
Use a compatible React/TypeScript project and a bundler supporting CSS. Component compatibility (from this manifest): react >=18, node >=18. Node applies to running the source installer, not the browser component. Copy for AI imposes no additional global version requirement. Runtime dependencies: react >=18 (reuse host), react-dom >=18 (reuse host). If the host is not React, explain the stack mismatch; do not convert the whole application. For SSR use a client boundary around interactive components.

1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/sidebar-navigation/releases/dba6b45d4384dc73/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 8d302f2bee70397e8fdd1e666d338218a245b3a444f2c0bff6068c37d5dcc5e6 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/sidebar-navigation` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/sidebar-navigation` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
