# Integrate AISEE CreditBar

Production delivery version: df361c2c42fc5d5b. Installer SHA-256: 88bbe714edada53232388f981ab5d73674488f7f9b502eee9038db793389079c
...
- CreditBar.tsx
- styles.css
- assets/credit-bar/sparkles.svg
- index.ts
- styles.css.d.ts
- assets/credit-bar/sparkles.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/credit-bar/releases/df361c2c42fc5d5b/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 88bbe714edada53232388f981ab5d73674488f7f9b502eee9038db793389079c before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/credit-bar` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/credit-bar` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
