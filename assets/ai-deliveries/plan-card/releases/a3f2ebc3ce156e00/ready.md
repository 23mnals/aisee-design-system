...
Production delivery version: a3f2ebc3ce156e00. Installer SHA-256: f14994d7f2d9f3feac62ceed24b8b415537c052cc46e61495ac5b3fad8d20e1a
...
- dialog: Dialog.tsx. Reference capabilities (only the requested missing subset applies to an existing target): controlled open/onClose, modal isolation, focus management and focus return; Escape/backdrop dismissal, accessible title/description and native dialog ref API; AISEE backdrop, entry/exit timing, layout variants and reduced-motion behavior. Fallback for an absent target only: standalone.
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/plan-card/releases/a3f2ebc3ce156e00/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 f14994d7f2d9f3feac62ceed24b8b415537c052cc46e61495ac5b3fad8d20e1a before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/plan-card` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/plan-card` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
