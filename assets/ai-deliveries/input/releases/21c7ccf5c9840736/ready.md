# Integrate AISEE Input

Production delivery version: 21c7ccf5c9840736. Installer SHA-256: 8cc072c2139d35310d6c996a953376d1be49c843eb8859214711f08b37f8a0ac
...
- input: Input.tsx. Reference capabilities (only the requested missing subset applies to an existing target): native input ref, value/defaultValue/onChange and form semantics; disabled, invalid, focus and accessible labeling. Fallback for an absent target only: standalone.
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/input/releases/21c7ccf5c9840736/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 8cc072c2139d35310d6c996a953376d1be49c843eb8859214711f08b37f8a0ac before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/input` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/input` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
