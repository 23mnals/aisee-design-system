# Integrate AISEE SegmentedChoice

Production delivery version: b675172f6159b615. Installer SHA-256: d0ed29fdd05475be04ea5011f1e03bc0e74c4fb707be0b6b9a45602fd0f11816
...
- single-choice: SegmentedChoice.tsx. Reference capabilities (only the requested missing subset applies to an existing target): controlled selection, disabled choices and keyboard activation; AISEE option layout and selected/focus hooks. Fallback for an absent target only: standalone.
...
- tooltip-position.ts
- styles.css
- assets/segmented-choice/info.svg
- index.ts
- styles.css.d.ts
- assets/segmented-choice/info.svg.d.ts
...
1. Complete Host Project Compatibility inspection and identify a requested missing capability first. If nothing is missing, make no changes. Only if reference source is needed, download https://23mnals.github.io/aisee-design-system/assets/ai-deliveries/segmented-choice/releases/b675172f6159b615/install.cjs as .aisee-install.cjs inside the host project; verify SHA-256 d0ed29fdd05475be04ea5011f1e03bc0e74c4fb707be0b6b9a45602fd0f11816 before execution. Do not print archive data into chat.
2. If the target component or styles already exist, run `node .aisee-install.cjs .aisee-reference/segmented-choice` only to inspect the verified source, extract only the requested missing capability into the existing host path while preserving its styles, library and interactions; for animation requests add only missing motion, and remove .aisee-reference afterwards. Do not create src/components/aisee or a second components/ui tree.
3. Only when the target component is absent and no host primitive provides the needed capability, run `node .aisee-install.cjs src/components/aisee/segmented-choice` (adapt the path) as a new standalone boundary following the host styling conventions. Never use this fallback to replace an existing target. The installer writes exactly the production list, installs no npm packages and refuses different existing files.
