...
- 绿色 Logo 外轮廓、眼白路径和黑色瞳孔形状保持原始数据不变；眨眼间隔约 2.2–3.2 秒，首次眨眼约 1.4–2 秒；闲置左右查看一轮约 2.6 秒，让动作更容易被观察到。
...
- SVG 资产与 React 组件均不提供外层背景；白色只属于眼白路径。
- 预览页的背景仅用于演示，可通过 `--preview-background` 或宿主页面覆盖；不要在 SVG 中增加白色背景矩形。
...
预览页头部可直接复制完整 TSX 源码；“开发交付”区域可一键下载包含 `AiseeLogoAnimation.tsx`、`preview.html` 和 `README.md` 的压缩包，也可分别下载三个真实源文件，不维护第二套副本。
