...
`name` 必须是侧边栏显示的英文名称，`html` 是必填的仓库相对路径，`category` 默认使用 `Explorations`，`subtitle` 必须说明其灵感参考边界；`image` 如果存在，仅作为资源登记和后续扩展使用。`source` / `surface` 记录来源，`designStatus` 记录使用状态，两者不得混为一个字段。新产物默认 `Draft`，不得因同步或展示自动升级为 `Selected` / `Current`。门户会把这些条目归到 Brand 分组，在详情页使用单个 HTML 预览，并可通过右上角 `Open HTML` 单独打开。
